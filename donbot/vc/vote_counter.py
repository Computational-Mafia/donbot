...
class VoteCounter:
    def __init__(self, slots, events=None, lessOneForMislynch=False, doublevoters=None):
    ...
    def kill_player(self, killed_player, post_number):
    ...
    def reset_player(self, reset_player, post_number):
        "Reset votes for a player"
        self.update_vote(reset_player, "UNVOTE", post_number)

    def reset_votecount(self, post_number):
        "Reset the votecount"
        for slot in self.slots:
            self.reset_player(slot[0], post_number)

    def update_vote(self, voter, voted, post_number):
